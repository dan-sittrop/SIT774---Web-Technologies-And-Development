...
{
    const {searchTerm: term, searchField: field} = req.body;

    const sql = `SELECT * FROM Members WHERE ${field} like '%${term}%'`; 
    ...
    db.all(sql, [], (err, rows) => {
        ...
        // Render the 'members' view, passing the members data from the search query
        res.render('search', {
